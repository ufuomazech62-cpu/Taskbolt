# -*- coding: utf-8 -*-
"""Logging setup for Taskbolt: console output and optional file handler."""
import io
import logging
...
_TASKBOLT_LOG_MAX_BYTES = 5 * 1024 * 1024  # 5 MiB
_TASKBOLT_LOG_BACKUP_COUNT = 3
...
def setup_logger(level: int | str = logging.INFO):
    """Configure logging to only output from this package (taskbolt), not deps."""
    ...
    # Only attach handler to our namespace so only taskbolt.* logs are printed.
...
def add_taskbolt_file_handler(log_path: Path) -> None:
    """Add a file handler to the taskbolt logger for daemon logs.
    ...
    Args:
        log_path: Path to the log file (e.g. WORKING_DIR / "taskbolt.log").
    """
    ...
    if is_windows_or_linux:
        ...
        file_handler = logging.handlers.RotatingFileHandler(
            log_path,
            encoding="utf-8",
            maxBytes=_TASKBOLT_LOG_MAX_BYTES,
            backupCount=_TASKBOLT_LOG_BACKUP_COUNT,
        )
