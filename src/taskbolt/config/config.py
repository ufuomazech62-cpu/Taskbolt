...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is Taskbolt. How can I help you?"
...
def load_agent_config(agent_id: str) -> AgentProfileConfig:
    ...
    # This keeps TASKBOLT_WORKING_DIR effective even if existing agent.json
    # contains older hard-coded paths like "~/.taskbolt/media".
    try:
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default Taskbolt agent",
    ...
    # When TASKBOLT_WORKING_DIR is customized, historical data may still exist
    # under "~/.taskbolt".
    old_workspace = Path("~/.taskbolt").expanduser().resolve()
