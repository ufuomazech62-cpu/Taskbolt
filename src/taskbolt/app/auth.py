...
variable ``TASKBOLT_AUTH_ENABLED`` is set to a truthy value (``true``,
...
def is_auth_enabled() -> bool:
    ...
    Returns ``True`` when ``TASKBOLT_AUTH_ENABLED`` is set to a truthy
...
def auto_register_from_env() -> None:
    ...
    Called once during application startup.  If ``TASKBOLT_AUTH_ENABLED``
    is truthy and both ``TASKBOLT_AUTH_USERNAME`` and ``TASKBOLT_AUTH_PASSWORD``
    ...
    username = os.environ.get("TASKBOLT_AUTH_USERNAME", "").strip()
    password = os.environ.get("TASKBOLT_AUTH_PASSWORD", "").strip()
