...
Shared by in-chat /daemon <sub> and CLI `taskbolt daemon <sub>`.
Logs: tail WORKING_DIR / "taskbolt.log". Restart: in-process reload of channels,
...
async def run_daemon_restart(context: DaemonContext) -> str:
    ...
    return (
        ...
        "Run the app (e.g. `taskbolt app`) and use /daemon restart in chat, "
...
def run_daemon_version(context: DaemonContext) -> str:
    ...
    return (
        ...
        f"- Log file: {context.working_dir / 'taskbolt.log'}"
    )


def run_daemon_logs(context: DaemonContext, lines: int = 100) -> str:
    """Tail last N lines from WORKING_DIR / taskbolt.log."""
    log_path = context.working_dir / "taskbolt.log"
