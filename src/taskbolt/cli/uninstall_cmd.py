# -*- coding: utf-8 -*-
"""taskbolt uninstall — remove the Taskbolt environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove Taskbolt PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# Taskbolt" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# Taskbolt\nexport PATH=\"\$HOME/\.taskbolt/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove Taskbolt environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL Taskbolt data in {wd}")
    else:
        click.echo(
            "This will remove the Taskbolt Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("Taskbolt uninstalled. Please restart your terminal.")
