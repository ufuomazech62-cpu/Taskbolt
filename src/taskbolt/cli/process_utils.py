...
def _matches_taskbolt_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a Taskbolt CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m taskbolt {subcommand}",
            f" taskbolt {subcommand}",
            f"__main__.py {subcommand}",
            f'taskbolt.exe" {subcommand}',
            f"taskbolt.exe {subcommand}",
        )
    )


def _is_taskbolt_service_command(command: str) -> bool:
    """Return whether the command line looks like a local Taskbolt app."""
    return _matches_taskbolt_cli_command(command, "app")


def _is_taskbolt_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a Taskbolt CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "taskbolt" or _matches_taskbolt_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local Taskbolt service."""
    if not host:
        return []
