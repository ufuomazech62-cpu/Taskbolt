# Taskbolt Desktop Application Guide
...
This guide explains how to install and use the Taskbolt Desktop application on Windows and macOS.

[releases]: https://github.com/agentscope-ai/Taskbolt/releases
...
1. **Download the installer**
   Download `Taskbolt-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\Taskbolt`
...
#### **Taskbolt Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "Taskbolt Desktop" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **Taskbolt Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "Taskbolt Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "Taskbolt Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**
A: Go to Windows Settings → Apps → Installed apps → Find "Taskbolt Desktop" → Uninstall
...
1. **Download the archive**
   Download `Taskbolt-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `Taskbolt.app`

3. **Move to Applications folder (Optional)**
   Drag `Taskbolt.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

Taskbolt is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `Taskbolt.app`
...
2. Scroll down to find a message like:
   _"'Taskbolt' was blocked from use because it cannot verify the developer"_
...
- Double-click `Taskbolt.app` to launch
- The app runs in the background and opens a browser window
- Logs are written to: `~/.taskbolt/desktop.log`
...
cd /Applications  # or wherever your Taskbolt.app is located

# Set environment variables and launch
APP_ENV="$(pwd)/Taskbolt.app/Contents/Resources/env"
PYTHONPATH= PYTHONHOME="$APP_ENV" "$APP_ENV/bin/python" -m taskbolt desktop
```
...
1. Check the `~/.taskbolt/desktop.log` file for errors
...
**Q: How to uninstall?**
A: Drag `Taskbolt.app` to the Trash, then delete the `~/.taskbolt` configuration folder
...
- **GitHub Issues**: [Submit an issue](https://github.com/agentscope-ai/Taskbolt/issues)
- **Packaging documentation**: `scripts/pack/README.md` - Technical details and local build guide
- **Log locations**:
  - Windows: View in Debug mode terminal, or `%USERPROFILE%\.taskbolt\` directory
  - macOS: `~/.taskbolt/desktop.log`
